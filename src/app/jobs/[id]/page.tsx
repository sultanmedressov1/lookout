      ...
      <div className="max-w-4xl mx-auto px-4 py-6">
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 space-y-5">
            ...
            <div className="bg-white rounded-xl border border-gray-200 p-6">
              <div className="flex items-start gap-4 mb-4">
                ...
                <div className="flex-1">
                  ...
                  {company && <Link href={`/company/${company.bin}`} className="text-blue-600 hover:text-blue-800 font-medium text-sm">{company.name_ru}</Link>}
                </div>
              </div>
              ...
              {job.salary_visible && (job.salary_from || job.salary_to) && (
                <div className="inline-flex items-center gap-2 bg-emerald-50 border border-emerald-200 text-emerald-800 px-4 py-2 rounded-lg text-sm font-medium">
                  {job.salary_from && job.salary_to ? `${fmt(job.salary_from)} – ${fmt(job.salary_to)} ₸/мес`
            ...
            {/* Форма отклика */}
            <ApplySection jobId={job.id} companyId={job.company_id} jobTitle={job.title} />
          </div>
          ...
          <div className="space-y-4">
            {company && (
              <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
                ...
                <div className="p-5">
                  <Link href={`/company/${company.bin}`} className="font-medium text-blue-600 hover:text-blue-800 text-sm block mb-2">{company.name_ru}</Link>
                  ...
                  <Link href={`/company/${company.bin}`} className="text-xs text-blue-600 hover:text-blue-800">
